...
{
    BNode* index_child = nullptr;
    BNode* index_prev_nd = nullptr;
    BNode* index_act_nd = nullptr;

    BNode* leaf_child = nullptr;
    BNode* leaf_prev_nd = nullptr;
    BNode* leaf_act_nd = nullptr;
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (leaf_act_nd == nullptr) {
            leaf_act_nd = new BNode();
            leaf_act_nd->init(0, this);      
            //  level 0, linked to this tree
    ...
    //  double linked list has been built

    //  build index nodes
    //  if last_end_block == last_start_block Stop building
    ...
    while (last_end_block > last_start_block) {
        ...
        for (int i = last_start_block; i <= last_end_block; i++) {
            block = i;
            //  get <block>
            //  this "if" is useless here actully
            if (current_level == 1) {
                //  its son is a **leaf**
                //  buff the key of this node in <value>
                leaf_child = new BNode();
